...
WFDD_CATEGORIES = ["grey_cloth", "grid_cloth", "pink_flower", "yellow_cloth"]

DATASET_NAME = "wfdd"
EPOCHS = 20
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
RESULTS_CSV = "results_wfdd_fpl_ad.csv"
...
def main():
    ...
    for i, category in enumerate(WFDD_CATEGORIES):
        if category in already_done:
            print(f"\n=== [{i+1}/{len(WFDD_CATEGORIES)}] {DATASET_NAME}/{category} — SKIPPED ===")
            continue

        print(f"\n=== [{i+1}/{len(WFDD_CATEGORIES)}] {DATASET_NAME}/{category} ===")
        start = time.time()
    ...
    if valid:
        ...
        print(f"\n=== MEAN across {len(valid)}/{len(WFDD_CATEGORIES)} categories ===")
        print(f"I-AUROC: {mean_i:.2f}")
        print(f"P-AUROC: {mean_p:.2f}")
        print(f"\nPaper's reported (Table 4b): I-AUROC 99.3 / P-AUROC 98.6")
