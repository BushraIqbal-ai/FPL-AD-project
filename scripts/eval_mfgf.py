...
def main():
    ...
    print(f"Evaluating (MFGF) on {DATASET_NAME}/{CATEGORY}: {len(test_ds)} test images")
    ...
    ckpt = torch.load(f"checkpoint_{DATASET_NAME}_{CATEGORY}_mfgf.pth", map_location=DEVICE)
    ...
    with torch.no_grad():
        for imgs, masks, labels in test_loader:
            ...
            x_prime = mfgf(x_adapted)   # <-- MFGF enhancement at inference too
    ...
    print(f"\nResults (MFGF) for {DATASET_NAME}/{CATEGORY}:")
    ...
    print(f"MFGF:     I-AUROC {i_auroc*100:.2f} / P-AUROC {p_auroc*100:.2f} / PRO {pro*100:.2f}")
