...
def main():
    ...
    print(f"Evaluating on {DATASET_NAME}/{CATEGORY}: {len(test_ds)} test images")
    ...
    ckpt = torch.load(f"checkpoint_{DATASET_NAME}_{CATEGORY}_baseline.pth", map_location=DEVICE)
    ...
    all_amaps_full = []   # for PRO: [N, H, W]
    all_masks_full = []   # for PRO: [N, H, W]
    ...
    with torch.no_grad():
        for imgs, masks, labels in test_loader:
            ...
            # ---- Image-level score: Top-k pooling ----
    ...
    # ---- PRO ----
    amaps_full = np.concatenate(all_amaps_full, axis=0)   # [N, H, W]
    masks_full = np.concatenate(all_masks_full, axis=0)   # [N, H, W]
    masks_full_bin = (masks_full > 0.5).astype(np.uint8)
    pro = compute_pro(masks_full_bin, amaps_full, num_thresholds=100)

    print(f"\nResults for {DATASET_NAME}/{CATEGORY}:")
