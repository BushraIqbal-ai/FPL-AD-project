...
RESULTS_CSV = "results_mpdd_fpl_ad.csv"
...
def main():
    ...
    for i, category in enumerate(MPDD_CATEGORIES):
        if category in already_done:
            print(f"\n=== [{i+1}/{len(MPDD_CATEGORIES)}] {DATASET_NAME}/{category} — SKIPPED ===")
            continue

        print(f"\n=== [{i+1}/{len(MPDD_CATEGORIES)}] {DATASET_NAME}/{category} ===")
        start = time.time()
    ...
    if valid:
        ...
        print(f"\n=== MEAN across {len(valid)}/{len(MPDD_CATEGORIES)} categories ===")
        print(f"I-AUROC: {mean_i:.2f}")
        print(f"P-AUROC: {mean_p:.2f}")
        print(f"PRO:     {mean_pro:.2f}")
        print(f"\nPaper's reported (Table 3): I-AUROC 98.8 / P-AUROC 99.0 / PRO 95.4")
